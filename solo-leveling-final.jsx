...
import { Home, BookOpen, Dumbbell, User, Plus, ChevronRight, Award, X, AlertTriangle, Zap, Clock, ListTodo, Trash2, ChevronDown, CheckCircle2, RefreshCw, Calendar, Brain } from 'lucide-react';
...
export default function SoloLevelingTracker() {
  ...
  const [taskForm, setTaskForm] = useState({ title: '', details: '', deadline: '', priority: 'medium' });
  
  const [formData, setFormData] = useState({ topic: '', minutes: '', confidence: 3, notes: '' });

  // Solo Leveling Ranks
  const ranks = [
    { name: 'Rank E', minXp: 0, maxXp: 299, icon: 'E', color: '#6B7280', bgColor: '#F3F4F6', borderColor: '#D1D5DB', title: 'Caçador Iniciante' },
    { name: 'Rank D', minXp: 300, maxXp: 799, icon: 'D', color: '#059669', bgColor: '#D1FAE5', borderColor: '#6EE7B7', title: 'Caçador Promissor' },
    { name: 'Rank C', minXp: 800, maxXp: 1799, icon: 'C', color: '#2563EB', bgColor: '#DBEAFE', borderColor: '#93C5FD', title: 'Caçador Competente' },
    { name: 'Rank B', minXp: 1800, maxXp: 3499, icon: 'B', color: '#7C3AED', bgColor: '#EDE9FE', borderColor: '#C4B5FD', title: 'Caçador de Elite' },
    { name: 'Rank A', minXp: 3500, maxXp: 5999, icon: 'A', color: '#DC2626', bgColor: '#FEE2E2', borderColor: '#FCA5A5', title: 'Caçador Nacional' },
  ...
  useEffect(() => {
    const saved = localStorage.getItem('solo-leveling-ebbinghaus-v2');
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('solo-leveling-ebbinghaus-v2', JSON.stringify({ studyData, exerciseData, tasks }));
  ...
  const getReviewStageName = (stage) => {
    const names = ['1ª Revisão (1 dia)', '2ª Revisão (7 dias)', '3ª Revisão (15 dias)', '4ª Revisão (30 dias)', '5ª Revisão (45 dias)'];
    if (stage < names.length) return names[stage];
    return `Revisão de Manutenção`;
  };

  const checkMissedDays = () => {
    const today = getToday();
    const lastEntry = exerciseData.history[0];
    ...
    if (lastEntry && lastEntry.date !== today) {
      ...
      if (diffDays > 1) {
        ...
        const xpPenalty = missedDays * 30;
        
        setStudyData(prev => ({
          ...prev,
          xp: Math.max(0, prev.xp - xpPenalty)
        }));
  ...
  const completeReview = () => {
    ...
    const session = {
      ...
      topic: `Revisão: ${selectedReview.topic}`,
      minutes: parseInt(formData.minutes),
      confidence: formData.confidence,
      notes: formData.notes,
      date: new Date().toISOString(),
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100">
      <div className="max-w-md mx-auto">
              ...
              {!punishmentCompleted ? (
                <>
                  <div className="text-center mb-6">
                    ...
                    <h3 className="text-2xl font-black text-red-600 mb-2">⚠️ PUNIÇÃO ⚠️</h3>
                    <p className="text-slate-600">Você pulou a revisão!</p>
                  </div>
                  
                  <div className="bg-red-50 rounded-2xl p-6 mb-6 border-2 border-red-200">
                    <p className="text-center text-slate-700 mb-2">Sua dívida:</p>
                    <div className="text-6xl font-black text-red-600 text-center mb-2">
                      {studyData.pushUpDebt}
                    </div>
                    <p className="text-center text-2xl font-bold text-red-500">FLEXÕES</p>
                  </div>

                  <div className="space-y-3">
                    <button 
                      onClick={completePushUps}
                      className="w-full py-4 rounded-2xl font-bold text-white bg-gradient-to-r from-green-500 to-emerald-600 shadow-lg"
                    >
                      ✅ COMPLETEI (+10 XP)
                    </button>
                    <button 
                      onClick={() => setShowPunishment(false)}
                      className="w-full py-3 rounded-xl text-slate-500 text-sm"
                    >
                      Lembrar depois
                    </button>
                  </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-600">Dívida quitada!</p>
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === 'home' && (
          <div className="p-4 pb-24 space-y-4">
            ...
            >
              ...
              <div className="mb-2">
                ...
                <div className="w-full bg-slate-200 rounded-full h-4 overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${progressInRank}%`, backgroundColor: currentRank.color }} 
                  />
                </div>
                <p className="text-xs text-slate-500 mt-1 text-right">
                  {nextRank ? `${xpToNext} XP para ${nextRank.name}` : '🏆 Rank Máximo!'}
                </p>
              </div>
              ...
              {studyData.pushUpDebt > 0 && (
                <button 
                  ...
                  className="w-full mt-4 py-3 rounded-xl bg-red-100 border-2 border-red-300 flex items-center justify-center gap-2"
                >
                  <AlertTriangle className="text-red-500" size={20} />
                  <span className="font-bold text-red-600">DÍVIDA: {studyData.pushUpDebt} FLEXÕES</span>
                </button>
              )}
            </div>
            ...
            <button
              ...
              className="w-full rounded-2xl p-4 bg-gradient-to-r from-emerald-500 to-green-600 shadow-lg flex items-center justify-center gap-3"
            >
            ...
            {pendingReviews.length > 0 && (
              <div className="rounded-3xl p-5 bg-white shadow-lg border-2 border-amber-300">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-slate-800">Revisões Pendentes</h3>
                ...
                <div className="space-y-3">
                  {pendingReviews.slice(0, 3).map(item => {
                    ...
                    return (
                      <div 
                        key={item.id} 
                        className={`rounded-2xl p-4 border-2 ${isLate ? 'bg-red-50 border-red-200' : 'bg-amber-50 border-amber-200'}`}
                      >
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <button 
                            onClick={() => { setSelectedReview(item); setShowStudyForm(true); }}
                            className="py-3 rounded-xl font-bold text-white bg-gradient-to-r from-amber-500 to-orange-500 flex items-center justify-center gap-2"
                          >
                            <CheckCircle2 size={18} />
                            Revisar
                          </button>
                          <button 
                            onClick={() => skipReview(item)}
                            className="py-3 rounded-xl font-bold text-red-600 bg-red-50 border-2 border-red-200"
                          >
                            Pular
                          </button>
                        </div>
                  ...
                  {pendingReviews.length > 3 && (
                    <p className="text-center text-sm text-amber-600 font-medium">
                      +{pendingReviews.length - 3} mais revisões pendentes
                    </p>
                  )}
                </div>
              </div>
            ...
            {pendingReviews.length === 0 && studyData.studyItems.length > 0 && (
              <div className="rounded-3xl p-6 bg-white shadow-lg border border-slate-200 text-center">
                ...
                <p className="text-sm text-slate-500">
                  Próxima revisão: {upcomingReviews[0] ? formatDate(upcomingReviews[0].nextReviewDate) : 'Nenhuma'}
                </p>
              </div>
            )}
            ...
            {studyData.studyItems.length === 0 && (
              <div className="rounded-3xl p-6 bg-white shadow-lg border border-slate-200 text-center">
                ...
                <p className="text-sm text-slate-500 mb-4">
                  Registre seu primeiro estudo para iniciar o sistema de revisão.
                </p>
              </div>
            )}
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              <div className="rounded-2xl p-4 bg-white shadow-lg border border-slate-200">
                <RefreshCw className="text-amber-500 mb-2" size={20} />
              ...
              <div className="rounded-2xl p-4 bg-white shadow-lg border border-slate-200">
                <Dumbbell className="text-purple-500 mb-2" size={20} />
                <p className="text-xs text-slate-500">Streak</p>
                <p className="text-2xl font-black text-slate-800">{exerciseData.currentStreak}🔥</p>
              </div>
            </div>
                    ...
                    return (
                      <div key={item.id} className="flex items-center justify-between text-sm">
                        ...
                        <span className={`text-xs font-medium px-2 py-1 rounded-full ml-2 ${
                          days <= 1 ? 'bg-amber-100 text-amber-700' : 'bg-slate-100 text-slate-600'
                        }`}>
              ...
              <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                <div className="w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="text-sm font-medium text-slate-600 mb-2 block">Matéria</label>
                      <select
                      ...
                      <>
                        <div className="bg-emerald-50 rounded-2xl p-4 border border-emerald-200">
                          ...
                          <p className="text-xs text-emerald-600 mt-1">
                            Revisões: 1 dia → 7 dias → 15 dias → 30 dias → 45 em 45
                          </p>
                        </div>
                        
                        <div>
                          <label className="text-sm font-medium text-slate-600 mb-1 block">Tópico estudado *</label>
                          <input 
                            type="text" 
                        ...
                        <div>
                          <label className="text-sm font-medium text-slate-600 mb-2 block">Nível de confiança</label>
            ...
            {showStudyForm && selectedReview && (
              <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                <div className="w-full max-w-md bg-white rounded-3xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto">
                  <div className="flex items-center justify-between mb-6">
                    <h3 className="text-2xl font-black text-slate-800">Revisão</h3>
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="text-sm font-medium text-slate-600 mb-1 block">Tempo de revisão (minutos) *</label>
                      <input 
                        type="number" 
                        value={formData.minutes} 
                        onChange={(e) => setFormData({ ...formData, minutes: e.target.value })} 
                    ...
                    <div>
                      <label className="text-sm font-medium text-slate-600 mb-2 block">Confiança após revisão</label>
                      <div className="flex gap-2">
                        {[1, 2, 3, 4, 5].map(l => (
                          <button 
                            key={l} 
        ...
        {activeTab === 'materias' && (
          <div className="p-4 pb-24 space-y-4">
            <h1 className="text-3xl font-black text-slate-800 mb-1">Matérias</h1>
            <p className="text-slate-500 mb-4">{studiedSubjects.length} matérias estudadas • {studyData.studyItems.length} tópicos</p>
            ...
            {subjects.map(subject => {
              ...
              return (
                <div 
                  key={subject} 
                  className={`rounded-2xl bg-white shadow-lg border-2 overflow-hidden ${
                    hasStudied ? 'border-slate-200' : 'border-slate-100 opacity-60'
                  }`}
                >
                  <button
                  ...
                  >
                    <div className="flex-1 text-left">
                      ...
                      <div className="flex gap-3 text-xs text-slate-500 mt-1">
                        {hasStudied ? (
                          <>
                            <span>{stats.items.length} tópicos</span>
                            <span>{stats.sessions.length} sessões</span>
                            <span>{stats.totalHours}h</span>
                          </>
                        ) : (
                          <span>Não estudada ainda</span>
                        )}
                      </div>
                    </div>
                    {hasStudied && (
                            ...
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                <p className="text-xs text-slate-500 mt-1">
                                  Estudado em {formatDate(item.studyDate)} • {item.reviews.length} revisões
                                </p>
                              </div>
                              <span className={`text-xs px-2 py-1 rounded-full font-medium ${
                                ...
                                {daysUntil < 0 ? `${Math.abs(daysUntil)}d atrasada` :
                                 daysUntil === 0 ? 'Hoje' :
                                 `em ${daysUntil}d`}
                              </span>
                            </div>
                            ...
                            {isItemExpanded && (
                              <div className="mt-3 pt-3 border-t border-slate-200 text-xs space-y-2">
                                <p className="text-slate-600">
                                  <strong>Próxima:</strong> {getReviewStageName(item.reviewStage)} em {formatDate(item.nextReviewDate)}
                                </p>
                                {item.reviews.length > 0 && (
                                  <div>
                                    <strong className="text-slate-600">Histórico:</strong>
                                    {item.reviews.map((r, i) => (
                                      <p key={i} className="text-slate-500 ml-2">
                                        {formatDate(r.date)} - {r.minutes}min - Confiança: {r.confidence}/5
                                      </p>
                                    ))}
                                  </div>
                                )}
                                {item.initialNotes && (
                                  <p className="text-slate-500">
                                    <strong>Notas:</strong> {item.initialNotes}
                                  </p>
                                )}
                              </div>
                            )}
        ...
        {activeTab === 'exercise' && (
          <div className="p-4 pb-24 space-y-4">
            <h1 className="text-3xl font-black text-slate-800 mb-2">Treino Diário</h1>
              ...
              <div className="text-center mb-4">
                ...
                >
                  {isTodayCompleted() ? (
                    ...
                    <Plus className="text-slate-400" size={48} />
                  )}
                </button>
                <p className="mt-3 text-lg font-bold text-slate-800">
                  {isTodayCompleted() ? '✅ Completo!' : 'Marcar treino'}
                </p>
              </div>
            </div>
            ...
            <div className="rounded-2xl p-5 bg-white shadow-lg border border-slate-200">
              <h3 className="text-lg font-bold text-slate-800 mb-4">Últimos 30 dias</h3>
              ...
              <div className="space-y-2 text-sm">
                {EBBINGHAUS_INTERVALS.map((days, i) => (
                  <div key={i} className="flex justify-between p-2 bg-slate-50 rounded-lg">
                    <span className="text-slate-700">{i + 1}ª Revisão</span>
                ...
                <div className="flex justify-between p-2 bg-green-50 rounded-lg">
                  <span className="text-slate-700">Manutenção</span>
